...
行业扩散 Claim 的确定性动作词覆盖 `adopted`、`deployed`、`deploys`、`to deploy`、`rolled out`、`rolling out` 和 `rolls out` 等已出现的词形；采用者名称可以出现在文档标题前缀之后，也可以出现在句子局部的 `Adopter becomes ... to deploy` 结构中。该词法补全只影响已配置、已验证的独立文档 Claim，不改变来源角色、四族证据门或 Ranking 的 fail-closed 条件；直接动作词没有匹配时，才继续尝试该有限 infinitive 结构，不会因缺少直接动词而跳过后续匹配。

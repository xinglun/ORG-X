...
当前路线图列出的 41 个 Work Item 均已有归档生命周期证据；本次状态修订完成并归档后，索引将包含 42 个 Work Item，Active Work Item 回到 0。后续新增工作必须从最新 `origin/main` 建立专用分支、创建独立 Contract，并通过 AI Cockpit Preflight；不能仅通过修改本路线图把新任务标记为完成。

...
) -> Result<String, ProductivityDomainError> {
    let value = value.into();
    if value.trim().is_empty() {
        return Err(ProductivityDomainError::EmptyValue { field });
    }
    Ok(value)
}
...
macro_rules! text_value {
    ($name:ident, $field:literal, $description:literal) => {
        ...
        impl $name {
            ...
            pub fn new(value: impl Into<String>) -> Result<Self, ProductivityDomainError> {

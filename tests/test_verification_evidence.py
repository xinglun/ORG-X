...
def test_v2_contract_rejects_raw_commands(command):
    ...
    assert any(
        "command is forbidden" in issue for issue in ai_check_work_item.validate_contract(contract)
    )
...
def test_registry_rejects_non_make_executable(tmp_path):
    ...
    with pytest.raises(ValueError, match="must invoke an explicit Make target"):
        ai_common.render_check_command(
            "evil", contract_path="c", summary_path="s", registry_path=registry
        )
...
def test_summary_validator_rejects_filename_and_work_item_mismatches():
    summary = {
        "summaryVersion": 2,
        "workItemId": "wrong",
...
def test_summary_diff_coverage_reports_missing_and_git_failure(monkeypatch):
    ...
    assert (
        "failed to read changed paths"
        in ai_check_summary.validate_changed_files_cover_diff({}, None)[0]
    )
